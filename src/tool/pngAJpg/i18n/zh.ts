...
const seo: PngAJpgLocaleContent['seo'] = [
  ...
  {
    "type": "paragraph",
    "html": "将 PNG 图片转换为 JPG 格式是提升网页加载速度、节省网络流量与存储空间的极佳方案。对于图片数量较多的电商网站、个人博客、作品集以及内容管理系统而言，合理使用压缩率更高的 JPG 格式能够显著改善 Google PageSpeed 得分与 Lighthouse 性能指标。本工具完全运行于本地浏览器环境，文件无需上传至远程服务器，在保证极速转换体验的同时，能够全面守护您的数据隐私与商业机密安全。无需安装任何额外插件与桌面软件，即开即用，是您日常处理与优化图像文件的理想利器与高效助手。欢迎随时使用本工具完成日常图片转换。"
  }
];
