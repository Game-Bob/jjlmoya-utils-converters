...
const seo: PngAJpgLocaleContent['seo'] = [
  ...
  {
    "type": "paragraph",
    "html": "PNG를 JPG로 변환하는 것은 단순한 형식 변경을 넘어 웹 사이트의 성능을 향상시키고 사용자 경험을 최적화하는 핵심적인 과정입니다. 특히 고해상도 이미지가 많은 블로그, 쇼핑몰, 포트폴리오 사이트에서는 압축률이 높은 JPG 형식을 사용하는 것이 페이지 로딩 속도를 줄이는 데 필수적입니다. 본 변환기는 사용자의 브라우저 내에서 100% 로컬로 작동하므로 소중한 사진과 개인 문서의 보안을 완벽하게 지켜줍니다. 외부 서버로 파일이 업로드되지 않아 빠른 속도와 완벽한 개인정보 보호를 동시에 제공하는 최상의 이미지 변환 솔루션입니다."
  }
];
